    ...
    try :
        ...
        if current_test.tobj.testobj.TestInputData.strace:
            execline = cfg.daytona_mon_path + "/sar_gather_agent.pl --daemonize --root-dir=" + current_test.statsdir + \
                       ...
                       " --strace-delay=" + str(current_test.tobj.testobj.TestInputData.strace_delay) + " --strace-duration=" + \
                       str(current_test.tobj.testobj.TestInputData.strace_duration)
        else:
            execline = cfg.daytona_mon_path + "/sar_gather_agent.pl --daemonize --root-dir=" + current_test.statsdir
...
def fileDownload(self, *args):
    ...
    try:
        ...
        shutil.rmtree(current_test.statsdir)
