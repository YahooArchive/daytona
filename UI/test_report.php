   ...
   <div class="content-wrapper" id="page-content">
        <div id="main-panel-alt-top">
          ...
          <div class="col-md-6 action-buttons-alt" id ="action-buttons-div">
            <button type="button" onclick="collapseAll()" class="btn btn-info btn-action">
              <i class="fa fa-minus-square fa-lg"></i>
              &nbsp;Collapse
            </button>
            <button type="button" onclick="expandAll()" class="btn btn-info btn-action">
...
foreach($report_arr as $rownum => $row_reports) {
  ...
  foreach($row_reports as $report_identifier) {
    $extension = pathinfo($report_identifier[0], PATHINFO_EXTENSION);
    $collapse_id++;
    ...
    echo "      <a data-toggle='collapse' class='collapse-link' href='#' onclick='collapseGraph(this)'>" .
         $report_identifier[1] . "</a>\n";
    echo "    </h4>\n";
    echo "  </div>\n";
    echo "  <div id='collapse$collapse_id' class='panel-collapse collapse graph-panel in no-transition'>\n";
    if (strpos($extension, 'plt') !== false){
    echo "    <div class='c3-graph-panel' id='c3item$collapse_id'></div>\n";
        $report_file = str_replace(".csv", ".plt", $report_identifier[0]);
      ...
      $full_paths = "";
        foreach($s_compids as $l_id) {
          ...
          $report_path = "test_data/" . $test_info_data["frameworkname"] . "/$l_id/results/" .
              formatFilePath($report_file, $test_info_data[$l_id]);
        ...
        echo "      <table class='table table-condensed sortable' id='c3footer$collapse_id'>\n";
        echo "        <thead>\n";
        echo "          <tr>\n";
        echo "            <th>Test</th>\n";
        echo "            <th>Min</th>\n";
        ...
        echo "      </table>\n";
        echo "    </div>\n";
    echo "    <script>buildTestReportGraphView('$collapse_id', '$full_paths', '$s_compids_str', '" . $report_identifier[1] . "', $err_threshold, '', '');</script>\n";
    echo "  </div>\n";
    ...
    if ($csv_col_count == 2){
       ...
       echo "    <div class='fixed-height' id=$div_id></div>\n";
       echo "    <script>buildFileToTableView('$full_paths', '$s_compids_str', '$div_id');</script>\n";
       echo "  </div>\n";
    }
    }else {
        $err_threshold = sizeof($s_compids);
    ...
    echo "    <script>buildTextCompareView('$full_paths', '$s_compids_str','$div_id');</script>\n";
        echo "  </div>\n";    
    }
...
?>
  ...
  <div class="panel panel-info panel-sub-main">
    <div class="panel-heading collapse-heading"><h4 class='panel-title text-center'>
      <a data-toggle='collapse' class='collapse-link' href='#' onclick='collapseGraph(this)'> Test Information
      </a></h4></div>
<?php
$collapse_id++;
echo "<div id='collapse$collapse_id' class='panel-collapse collapse graph-panel in no-transition'>\n";
...
<?php printRowFields($allTestData, 'cc_list'); ?>
 ...
 <div>
  <div class="panel panel-info panel-sub-main">
    <div class="panel-heading collapse-heading"><h4 class='panel-title text-center'>
    <a data-toggle='collapse' class='collapse-link' href='#' onclick='collapseGraph(this)'> Run Status</a></h4></div>
    <?php
      $collapse_id++;
      echo "<div id='collapse$collapse_id' class='panel-collapse collapse graph-panel in no-transition'>\n";
      ?>
...
<?php printRowFields($allTestData, 'state_detail'); ?>
  ...
  <div class="panel panel-info panel-sub-main">
    <div class="panel-heading collapse-heading"><h4 class='panel-title text-center'>
      <a data-toggle='collapse' class='collapse-link' href='#' onclick='collapseGraph(this)'> Test Arguments</a></h4></div>
      <?php
        $collapse_id++;
        echo "<div id='collapse$collapse_id' class='panel-collapse collapse graph-panel in no-transition'>\n";
      ?>
