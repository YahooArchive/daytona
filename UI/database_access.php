...
$testid = "";
if($_GET["query"] == "killTest" || $_GET["query"] == "runTest") {
...
if(isset($_GET["frameworkid"])) {
  $framework_clause_and = "AND frameworkid='" . $conn->real_escape_string($_GET["frameworkid"]) . "'";
  $framework_clause_where = "WHERE frameworkid='" . $conn->real_escape_string($_GET["frameworkid"]) . "'";
}
...
switch($_GET["query"]) {
  case "killTest":
    $sql_query = "DELETE FROM CommonFrameworkSchedulerQueue WHERE " .
                 "testid=$testid AND state='scheduled';" .
                 "UPDATE TestInputData SET end_status='kill' WHERE " .
                 "testid=$testid;" .
                 "UPDATE CommonFrameworkSchedulerQueue SET state='kill' WHERE " .
                 "testid=$testid AND state!='scheduled'";
    break;
  case "runTest":
